...
| Topic | Base pipe `\|>` (this book) | magrittr `%>%` (inherited code) |
|---|---|---|
...
| Other argument | Placeholder `_`: `mtcars \|> lm(mpg ~ disp, data = _)` | Dot `.`: `mtcars %>% lm(mpg ~ disp, data = .)` |
| Inline function | `\(x)`: `1:5 \|> (\(x) x^2)() \|> sum()` | `.` shorthand: `1:5 %>% (function(x) x^2) %>% sum()` |

For first-argument chains — every example in this book — the two pipes read
identically, so inherited `%>%` code can be read as `|>`:
