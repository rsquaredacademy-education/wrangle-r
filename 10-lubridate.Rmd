...
- check whether time occurs in AM or PM?
...
```{r showing, echo=FALSE, eval=TRUE, message=FALSE}
transact <- readr::read_csv('https://raw.githubusercontent.com/rsquaredacademy/datasets/master/transact.csv')
transact
```
...
## Date & Time Classes {#datetimeclass}
...
```{r img1iso, fig.align='center', echo=FALSE}
...
```{r img2duration, echo=FALSE, out.width="100%", fig.align="center"}
...
```{r img3shiftdates, echo=FALSE, out.width="100%", fig.align="center"}
...
- Check the time in a different time zone 
...
- Specify the format for the RStudio release dates
...
If you want the name of the month instead, use the `label` argument and set it 
...
- create R release date
- update R release date
- create football world cup years 
...
- round R release dates
- rollback R release dates